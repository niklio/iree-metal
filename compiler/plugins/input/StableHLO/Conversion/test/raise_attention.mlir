...
// RUN: env IREE_STABLEHLO_RAISE_ATTENTION=1 iree-opt --split-input-file --pass-pipeline="builtin.module(inline,func.func(canonicalize,iree-stablehlo-canonicalize,cse),iree-stablehlo-convert-flash-attention-dispatch)" %s | FileCheck %s --check-prefix=OPTION-OFF
// RUN: env IREE_STABLEHLO_RAISE_ATTENTION=1 iree-opt --split-input-file --pass-pipeline="builtin.module(inline,func.func(canonicalize,iree-stablehlo-canonicalize,cse),iree-stablehlo-convert-flash-attention-dispatch{raise-native-attention=true},func.func(canonicalize,iree-stablehlo-canonicalize,cse))" %s | FileCheck %s --check-prefixes=RAISED,NEGATIVE
// RUN: env -u IREE_STABLEHLO_RAISE_ATTENTION iree-opt --split-input-file --iree-stablehlo-input-transformation-pipeline %s | FileCheck %s --check-prefix=PIPELINE-OFF
// RUN: env IREE_STABLEHLO_RAISE_ATTENTION=1 iree-opt --split-input-file --iree-stablehlo-input-transformation-pipeline %s | FileCheck %s --check-prefixes=PIPELINE,PIPELINE-NEGATIVE
...
// are reduced to 2, 2, 4, and 8, but the rank-4 dot dimension numbers, outlined
...
// RAISED-SAME: %[[Q:[^:]+]]: tensor<2x2x4x8xbf16>,
// RAISED-SAME: %[[K:[^:]+]]: tensor<2x2x4x8xbf16>,
// RAISED-SAME: %[[V:[^:]+]]: tensor<2x2x4x8xbf16>,
// RAISED-SAME: {{%[^:]+}}: tensor<2x2x4x8xbf16>)
// RAISED: %[[SCALE:.+]] = arith.constant {{.*}} : bf16
// RAISED: %[[MASK:.+]] = stablehlo.select
// RAISED-NOT: stablehlo.exponential
// RAISED-NOT: tensor<2x2x4x4xbf16>
...
// NEGATIVE-LABEL: func.func public @wrong_backward_scale
...
// PIPELINE-NEGATIVE-LABEL: func.func public @wrong_backward_scale
...
// CURRENT-SAME: %[[Q:[^:]+]]: tensor<2x2x4x8xbf16>,
// CURRENT-SAME: %[[K:[^:]+]]: tensor<2x2x4x8xbf16>,
// CURRENT-SAME: %[[V:[^:]+]]: tensor<2x2x4x8xbf16>,
// CURRENT-SAME: %[[DO:[^:]+]]: tensor<2x2x4x8xbf16>)
...
// CURRENT-SAME: -> tensor<2x2x4x8xbf16>
...
// CURRENT: stablehlo.constant dense<0.000000e+00> : tensor<2x2x4x4xbf16>
// CURRENT: stablehlo.select

module @jit_loss attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<2x2x4x8xbf16>, %arg1: tensor<2x2x4x8xbf16>, %arg2: tensor<2x2x4x8xbf16>, %arg3: tensor<2x2x4x8xbf16>) -> (tensor<f32> {jax.result_info = "result[0]"}, tensor<2x2x4x8xbf16> {jax.result_info = "result[1][0]"}, tensor<2x2x4x8xbf16> {jax.result_info = "result[1][1]"}, tensor<2x2x4x8xbf16> {jax.result_info = "result[1][2]"}) {
    %0 = stablehlo.transpose %arg1, dims = [0, 1, 3, 2] : (tensor<2x2x4x8xbf16>) -> tensor<2x2x8x4xbf16>
    %1 = stablehlo.dot_general %arg0, %0, batching_dims = [0, 1] x [0, 1], contracting_dims = [3] x [2], precision = [DEFAULT, DEFAULT] : (tensor<2x2x4x8xbf16>, tensor<2x2x8x4xbf16>) -> tensor<2x2x4x4xbf16>
    %cst = stablehlo.constant dense<3.535160e-01> : tensor<bf16>
    %2 = stablehlo.broadcast_in_dim %cst, dims = [] : (tensor<bf16>) -> tensor<2x2x4x4xbf16>
    %3 = stablehlo.multiply %1, %2 : tensor<2x2x4x4xbf16>
    %c = stablehlo.constant dense<true> : tensor<i1>
    %4 = stablehlo.broadcast_in_dim %c, dims = [] : (tensor<i1>) -> tensor<4x4xi1>
    %5 = call @tril(%4) : (tensor<4x4xi1>) -> tensor<4x4xi1>
    %6 = stablehlo.broadcast_in_dim %5, dims = [2, 3] : (tensor<4x4xi1>) -> tensor<1x1x4x4xi1>
    %cst_0 = stablehlo.constant dense<-3.389530e+38> : tensor<bf16>
    %7:2 = call @_where(%6, %3, %cst_0) : (tensor<1x1x4x4xi1>, tensor<2x2x4x4xbf16>, tensor<bf16>) -> (tensor<2x2x4x4xbf16>, tensor<2x2x4x4xi1>)
    %cst_1 = stablehlo.constant dense<0xFF80> : tensor<bf16>
    %8 = stablehlo.reduce(%7#0 init: %cst_1) applies stablehlo.maximum across dimensions = [3] : (tensor<2x2x4x4xbf16>, tensor<bf16>) -> tensor<2x2x4xbf16>
    %cst_2 = stablehlo.constant dense<0xFF80> : tensor<bf16>
    %9 = stablehlo.broadcast_in_dim %cst_2, dims = [] : (tensor<bf16>) -> tensor<2x2x4xbf16>
    %10 = stablehlo.maximum %9, %8 : tensor<2x2x4xbf16>
    %11 = stablehlo.broadcast_in_dim %10, dims = [0, 1, 2] : (tensor<2x2x4xbf16>) -> tensor<2x2x4x1xbf16>
    %12 = stablehlo.broadcast_in_dim %11, dims = [0, 1, 2, 3] : (tensor<2x2x4x1xbf16>) -> tensor<2x2x4x4xbf16>
    %13 = stablehlo.subtract %7#0, %12 : tensor<2x2x4x4xbf16>
    %14 = stablehlo.exponential %13 : tensor<2x2x4x4xbf16>
    %15 = stablehlo.convert %14 : (tensor<2x2x4x4xbf16>) -> tensor<2x2x4x4xf32>
    %cst_3 = stablehlo.constant dense<0.000000e+00> : tensor<f32>
    %16 = stablehlo.reduce(%15 init: %cst_3) applies stablehlo.add across dimensions = [3] : (tensor<2x2x4x4xf32>, tensor<f32>) -> tensor<2x2x4xf32>
    %17 = stablehlo.broadcast_in_dim %16, dims = [0, 1, 2] : (tensor<2x2x4xf32>) -> tensor<2x2x4x1xf32>
    %18 = stablehlo.convert %17 : (tensor<2x2x4x1xf32>) -> tensor<2x2x4x1xbf16>
    %19 = stablehlo.broadcast_in_dim %18, dims = [0, 1, 2, 3] : (tensor<2x2x4x1xbf16>) -> tensor<2x2x4x4xbf16>
    %20 = stablehlo.divide %14, %19 : tensor<2x2x4x4xbf16>
    %21 = stablehlo.multiply %18, %18 : tensor<2x2x4x1xbf16>
    %cst_4 = stablehlo.constant dense<1.000000e+00> : tensor<bf16>
    %22 = stablehlo.broadcast_in_dim %cst_4, dims = [] : (tensor<bf16>) -> tensor<2x2x4x1xbf16>
    %23 = stablehlo.divide %22, %21 : tensor<2x2x4x1xbf16>
    %24 = stablehlo.dot_general %20, %arg2, batching_dims = [0, 1] x [0, 1], contracting_dims = [3] x [2], precision = [DEFAULT, DEFAULT] : (tensor<2x2x4x4xbf16>, tensor<2x2x4x8xbf16>) -> tensor<2x2x4x8xbf16>
    %25 = stablehlo.convert %24 : (tensor<2x2x4x8xbf16>) -> tensor<2x2x4x8xf32>
    %26 = stablehlo.convert %arg3 : (tensor<2x2x4x8xbf16>) -> tensor<2x2x4x8xf32>
    %27 = stablehlo.multiply %25, %26 : tensor<2x2x4x8xf32>
    %cst_5 = stablehlo.constant dense<0.000000e+00> : tensor<f32>
    %28 = stablehlo.reduce(%27 init: %cst_5) applies stablehlo.add across dimensions = [0, 1, 2, 3] : (tensor<2x2x4x8xf32>, tensor<f32>) -> tensor<f32>
    %cst_6 = stablehlo.constant dense<1.000000e+00> : tensor<f32>
    %29 = stablehlo.broadcast_in_dim %cst_6, dims = [] : (tensor<f32>) -> tensor<2x2x4x8xf32>
    %30 = stablehlo.multiply %29, %26 : tensor<2x2x4x8xf32>
    %31 = stablehlo.convert %30 : (tensor<2x2x4x8xf32>) -> tensor<2x2x4x8xbf16>
    %32 = stablehlo.dot_general %31, %20, batching_dims = [0, 1] x [0, 1], contracting_dims = [2] x [2], precision = [DEFAULT, DEFAULT] : (tensor<2x2x4x8xbf16>, tensor<2x2x4x4xbf16>) -> tensor<2x2x8x4xbf16>
    %33 = stablehlo.transpose %32, dims = [0, 1, 3, 2] : (tensor<2x2x8x4xbf16>) -> tensor<2x2x4x8xbf16>
    %34 = stablehlo.dot_general %31, %arg2, batching_dims = [0, 1] x [0, 1], contracting_dims = [3] x [3], precision = [DEFAULT, DEFAULT] : (tensor<2x2x4x8xbf16>, tensor<2x2x4x8xbf16>) -> tensor<2x2x4x4xbf16>
    %35 = stablehlo.broadcast_in_dim %23, dims = [0, 1, 2, 3] : (tensor<2x2x4x1xbf16>) -> tensor<2x2x4x4xbf16>
    %36 = stablehlo.multiply %34, %35 : tensor<2x2x4x4xbf16>
    %37 = stablehlo.multiply %36, %14 : tensor<2x2x4x4xbf16>
    %cst_7 = stablehlo.constant dense<0.000000e+00> : tensor<bf16>
    %38 = stablehlo.reduce(%37 init: %cst_7) applies stablehlo.add across dimensions = [3] : (tensor<2x2x4x4xbf16>, tensor<bf16>) -> tensor<2x2x4xbf16>
    %39 = stablehlo.reshape %38 : (tensor<2x2x4xbf16>) -> tensor<2x2x4x1xbf16>
    %40 = stablehlo.negate %39 : tensor<2x2x4x1xbf16>
    %41 = stablehlo.broadcast_in_dim %18, dims = [0, 1, 2, 3] : (tensor<2x2x4x1xbf16>) -> tensor<2x2x4x4xbf16>
    %42 = stablehlo.divide %34, %41 : tensor<2x2x4x4xbf16>
    %43 = stablehlo.convert %40 : (tensor<2x2x4x1xbf16>) -> tensor<2x2x4x1xf32>
    %cst_8 = stablehlo.constant dense<0.000000e+00> : tensor<f32>
    %44 = stablehlo.reduce(%43 init: %cst_8) applies stablehlo.add across dimensions = [3] : (tensor<2x2x4x1xf32>, tensor<f32>) -> tensor<2x2x4xf32>
    %45 = stablehlo.broadcast_in_dim %44, dims = [0, 1, 2] : (tensor<2x2x4xf32>) -> tensor<2x2x4x4xf32>
    %46 = stablehlo.convert %45 : (tensor<2x2x4x4xf32>) -> tensor<2x2x4x4xbf16>
    %47 = stablehlo.add %42, %46 : tensor<2x2x4x4xbf16>
    %48 = stablehlo.multiply %47, %14 : tensor<2x2x4x4xbf16>
    %49 = call @_where_0(%7#1, %48) : (tensor<2x2x4x4xi1>, tensor<2x2x4x4xbf16>) -> tensor<2x2x4x4xbf16>
    %50 = stablehlo.broadcast_in_dim %cst, dims = [] : (tensor<bf16>) -> tensor<2x2x4x4xbf16>
    %51 = stablehlo.multiply %49, %50 : tensor<2x2x4x4xbf16>
    %52 = stablehlo.dot_general %51, %arg0, batching_dims = [0, 1] x [0, 1], contracting_dims = [2] x [2], precision = [DEFAULT, DEFAULT] : (tensor<2x2x4x4xbf16>, tensor<2x2x4x8xbf16>) -> tensor<2x2x4x8xbf16>
    %53 = stablehlo.transpose %52, dims = [0, 1, 3, 2] : (tensor<2x2x4x8xbf16>) -> tensor<2x2x8x4xbf16>
    %54 = stablehlo.dot_general %51, %0, batching_dims = [0, 1] x [0, 1], contracting_dims = [3] x [3], precision = [DEFAULT, DEFAULT] : (tensor<2x2x4x4xbf16>, tensor<2x2x8x4xbf16>) -> tensor<2x2x4x8xbf16>
    %55 = stablehlo.transpose %53, dims = [0, 1, 3, 2] : (tensor<2x2x8x4xbf16>) -> tensor<2x2x4x8xbf16>
    return %28, %54, %55, %33 : tensor<f32>, tensor<2x2x4x8xbf16>, tensor<2x2x4x8xbf16>, tensor<2x2x4x8xbf16>
  }

  // This is a complete canonical forward/VJP spelling except that dS uses a
  // different scale. Matching reaches the late shared-scale proof and must
  // leave both the forward and backward untouched.
  func.func public @wrong_backward_scale(
    ...
    %scale = stablehlo.constant dense<3.535160e-01> :
        tensor<2x2x4x4xbf16>
    %wrong_scale = stablehlo.constant dense<5.000000e-01> :
        tensor<2x2x4x4xbf16>
    ...
    %masked_ds = stablehlo.select %mask, %unmasked_ds, %zero_scores :
        tensor<2x2x4x4xi1>, tensor<2x2x4x4xbf16>
    %ds = stablehlo.multiply %masked_ds, %wrong_scale :
        tensor<2x2x4x4xbf16>
  ...
  func.func private @tril(%arg0: tensor<4x4xi1>) -> tensor<4x4xi1> {
    %0 = stablehlo.iota dim = 0 : tensor<4x4xi32>
    %c = stablehlo.constant dense<0> : tensor<i32>
    %1 = stablehlo.broadcast_in_dim %c, dims = [] : (tensor<i32>) -> tensor<4x4xi32>
    %2 = stablehlo.add %0, %1 : tensor<4x4xi32>
    %3 = stablehlo.iota dim = 1 : tensor<4x4xi32>
    %4 = stablehlo.compare  GE, %2, %3,  SIGNED : (tensor<4x4xi32>, tensor<4x4xi32>) -> tensor<4x4xi1>
    %c_0 = stablehlo.constant dense<false> : tensor<i1>
    %5 = stablehlo.broadcast_in_dim %c_0, dims = [] : (tensor<i1>) -> tensor<4x4xi1>
    %6 = stablehlo.select %4, %arg0, %5 : tensor<4x4xi1>, tensor<4x4xi1>
    return %6 : tensor<4x4xi1>
  }
  func.func private @_where(%arg0: tensor<1x1x4x4xi1>, %arg1: tensor<2x2x4x4xbf16>, %arg2: tensor<bf16>) -> (tensor<2x2x4x4xbf16>, tensor<2x2x4x4xi1>) {
    %0 = stablehlo.broadcast_in_dim %arg0, dims = [0, 1, 2, 3] : (tensor<1x1x4x4xi1>) -> tensor<2x2x4x4xi1>
    %1 = stablehlo.broadcast_in_dim %arg2, dims = [] : (tensor<bf16>) -> tensor<2x2x4x4xbf16>
    %2 = stablehlo.select %0, %arg1, %1 : tensor<2x2x4x4xi1>, tensor<2x2x4x4xbf16>
    return %2, %0 : tensor<2x2x4x4xbf16>, tensor<2x2x4x4xi1>
  }
  func.func private @_where_0(%arg0: tensor<2x2x4x4xi1>, %arg1: tensor<2x2x4x4xbf16>) -> tensor<2x2x4x4xbf16> {
    %cst = stablehlo.constant dense<0.000000e+00> : tensor<bf16>
    %0 = stablehlo.broadcast_in_dim %cst, dims = [] : (tensor<bf16>) -> tensor<2x2x4x4xbf16>
    %1 = stablehlo.select %arg0, %arg1, %0 : tensor<2x2x4x4xi1>, tensor<2x2x4x4xbf16>
    return %1 : tensor<2x2x4x4xbf16>
  }
}
