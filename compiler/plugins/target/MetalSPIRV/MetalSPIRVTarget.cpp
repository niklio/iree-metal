...
public:
  ...
  LogicalResult serializeExecutable(const SerializationOptions &serOptions,
    ...
    // External variants carry hand-authored MSL objects instead of a
    // compiler-generated spirv.module (used by custom_dispatch). Mirror the
    // VulkanSPIRV target's external path: embed the provided source directly
    // rather than running codegen + spirv-cross.
      ...
      if (hostTriple.isMacOSX()) {
        for (auto [i, shader, entryPoint] :
          ...
          std::unique_ptr<llvm::MemoryBuffer> lib = compileMSLToMetalLib(
              options.targetPlatform, shader.languageVersion, shader.source,
              entryPoint);
          if (!lib) {
            return variantOp.emitError()
    ...
    for (auto [shader, metallib] : llvm::zip_equal(mslShaders, metallibs)) {
      ...
      if (embedSource) {
        ...
        sourceRef = iree_hal_metal_MSLSourceDef_create(
            builder, shader.languageVersion, sourceStrRef);
      }
  ...
  // Serializes an external executable variant: one whose kernel is provided as a
  // hand-authored MSL object (no inner spirv.module). Embeds the MSL source into
  // the Metal flatbuffer and emits a PipelineDef per export from its layout.
  // Mirrors VulkanSPIRVTarget::serializeExternalExecutable.
  LogicalResult
  serializeExternalExecutable(const SerializationOptions &serOptions,
    ...
    if (!variantOp.getObjects().has_value()) {
      ...
      // For now we assume exactly one MSL object file per variant.
      return variantOp.emitOpError() << "only one object reference is "
                                        "supported for external variants";
    }

    // Load the MSL source object file.
    ...
    // One library carrying the provided MSL source.
    SmallVector<iree_hal_metal_LibraryDef_ref_t> libraryRefs;
    {
      unsigned version = 196608; // MTLLanguageVersion3_0
      auto sourceStrRef = builder.createString(mslSource);
      auto sourceRef =
    ...
    SmallVector<iree_hal_metal_PipelineDef_ref_t> pipelineRefs;
    for (auto [i, exportOp] : llvm::enumerate(exportOps)) {
      auto entryPointRef = builder.createString(exportOp.getName());

      // Threadgroup size from the export's optional workgroup_size attr,
      // defaulting to (64,1,1). The host count() region computes the number of
      // threadgroups; this is the per-group thread count.
      iree_hal_metal_ThreadgroupSize_t threadgroupSize = {64, 1, 1};
      if (auto wgsAttr = exportOp.getWorkgroupSizeAttr()) {
        auto dims = wgsAttr.getValue();
        if (dims.size() >= 1)
          threadgroupSize.x = cast<IntegerAttr>(dims[0]).getInt();
        if (dims.size() >= 2)
          threadgroupSize.y = cast<IntegerAttr>(dims[1]).getInt();
        if (dims.size() >= 3)
          threadgroupSize.z = cast<IntegerAttr>(dims[2]).getInt();
      }
      ...
      iree_hal_metal_PipelineDef_entry_point_add(builder, entryPointRef);
      iree_hal_metal_PipelineDef_threadgroup_size_add(builder, &threadgroupSize);
      iree_hal_metal_PipelineDef_constant_count_add(builder, constantCount);
      iree_hal_metal_PipelineDef_binding_flags_add(builder, bindingFlagsRef);
      iree_hal_metal_PipelineDef_debug_info_add(builder, exportDebugInfos[i]);
      pipelineRefs.push_back(iree_hal_metal_PipelineDef_end(builder));
